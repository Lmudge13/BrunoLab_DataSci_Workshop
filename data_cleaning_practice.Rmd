...
data[1:20, ] %>% 
  select(reef_id, date, substrate_code, segment_code, total) %>%
...
# use arrange to change the order of how variables appear in the column (ie alphabetical or numerical order)
data[1:20, ] %>% 
  select(reef_id, date, substrate_code, segment_code, total) %>%
  arrange(substrate_code, total)
...
# Example of lots of duplicates:
lots_dupes <- data_by_survey %>%
  group_by(reef_id, date) %>%
  mutate(coral = round(mean(perc_survey), 2),
         coral_std = round(sd(perc_survey), 2),
         depth_m = round(mean(depth), 2),
